...
class Test3 extends Component {
    ...
    handleRemove = (id) => {
        this.setState({
            movies : this.state.movies.filter(movies => movies.no !== id )
        })
    }
    render() {
        const {no,poster,actor,director,date} = this.state
        return (
            <div className="gallery">
                {/* <article>
                    ...
                    <div className="right">
                        ...
                        <ul>
                            <li>Actor : {this.state.movies[0].actor}</li>
                            <li>Director : {this.state.movies[0].director}</li>
                            <li>Date : {this.state.movies[0].date}</li>
                        </ul>
                        <button>del</button>
                    </div>
                </article> */}
                    ...
                    this.state.movies.map(movies => 
                        <article ket={movies.no}>
                            ...
                            <div className="right">
                                <h3>{movies.title}</h3>
                                <ul>
                                    <li>Actor : {movies.actor}</li>
                                    <li>Director : {movies.director}</li>
                                    <li>Date : {movies.date}</li>
                                </ul>
                                <button onClick={() => this.handleRemove(movies.no)}>del</button>
                            </div>
                        </article>)
                }
            </div>
